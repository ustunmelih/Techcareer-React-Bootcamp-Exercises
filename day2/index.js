...
productOn1998 = orders.filter((order) => {
  if (
    order.orderDate.slice(0, 4) == '1998' &&
    order.orderDate.slice(5, 7) == '05'
  ) {
    return order
  }
})

//total price of product on 1998
let totalPrice = 0
productOn1998.forEach((order) => {
  if (order.details) {
    order.details.forEach((detail) => {
      totalPrice += detail.quantity * detail.unitPrice
      totalPrice = +totalPrice.toFixed()
